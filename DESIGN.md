...
- **Tokens propios del panel:** `--surface #111111`, `--surface-2 #171717`, `--hover #1C1C1C`, `--line #242424`, `--line-strong #363636`, `--muted #A3A39B` (texto secundario, contraste ≥4.5:1), `--faint #7E7E77` (solo placeholders/estados vacíos, contraste ≥4.5:1 contra `--surface`), hover del acento `#F0B052`, burbuja de Dominga `#1B1509` con borde `#3A2B10`. Estados: `--ok` (contacto, en vivo), `--alert` (escalada).
...
- **Color e identidad por canal:** cada conversación muestra el color de marca oficial de su origen — `--ch-whatsapp #25D366`, `--ch-messenger #0084FF`, `--ch-instagram #E1306C` (etiquetas) con `--ch-instagram-grad` (`linear-gradient(45deg,#F09433,#E6683C,#DC2743,#CC2366,#BC1888)`, el degradado oficial de Instagram) reservado para el avatar-iniciales de ese canal, y `--ch-web` (`var(--muted)`) para el sitio. Son colores de marca de terceros, no del sistema propio — no se sustituyen por tokens del panel ni se usan fuera de esta identificación de canal (etiquetas y avatares).
- **Avatar de contacto:** foto real si Meta la entrega (Instagram/Messenger), o iniciales sobre el color del canal si no — WhatsApp no expone foto de perfil vía API, así que ese canal siempre muestra iniciales.
...
- **Do** incluir el guard `@media (prefers-reduced-motion: reduce)` (anula duración de animaciones/transiciones a 0.01ms) en cualquier `<style>` nuevo del sitio — landing, páginas legales/utilitarias y `/admin` lo tienen los tres; una página nueva que agregue su propio `<style>` debe copiarlo también.
