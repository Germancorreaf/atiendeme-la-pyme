...
El chatbot opera en varios canales: sitio web, WhatsApp Cloud API directo (`src/api/whatsapp.js`), e Instagram/Messenger vía integración nativa directa con Meta (`src/api/meta-connect.js` + `src/api/meta-webhook.js`), que reemplaza a ManyChat (retirado del código, incluido el endpoint `/api/chat-manychat`) y está pendiente de probar de punta a punta con la cuenta real (ver TODOS.md). Agenda citas directo en Google Calendar, sin dobles reservas, y envía recordatorios automáticos un día antes (el copy de precios también menciona Agenda Pro como opción para el plan Pro del cliente, pero esa integración todavía no tiene código — ver TODOS.md). Un dashboard interno (protegido con login por sesión, cookie HMAC) permite revisar conversaciones, leads y agenda manualmente, y además tomar una conversación en vivo cuando el bot escala.
