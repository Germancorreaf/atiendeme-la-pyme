...
<div id="atp-chat-wrapper">
  <button id="atp-chat-btn" class="atp-chat-btn-premium" aria-label="Abrir chat">
    ...
    <span class="atp-pulse"></span>
  </button>

  <div id="atp-chat-panel" class="atp-chat-panel-premium">
    <div class="atp-chat-header-premium">
      <div class="atp-header-left">
        <div class="atp-avatar-premium">👩‍💻</div>
        <div class="atp-header-text">
          <div class="atp-title">Dominga</div>
          <div class="atp-subtitle">Disponible 24/7</div>
        </div>
      </div>
      <button id="atp-chat-close" class="atp-close-btn" aria-label="Cerrar chat">✕</button>
    </div>
    
    <div id="atp-chat-messages" class="atp-messages-container"></div>
    
    <div class="atp-input-section">
      <div class="atp-input-wrapper">
        <input 
          id="atp-chat-input" 
          type="text" 
          class="atp-input-premium"
          placeholder="Escribe tu pregunta..." 
          autocomplete="off"
        />
        <button id="atp-chat-send" class="atp-send-btn-premium" aria-label="Enviar">
          <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round">
            <path d="M5 12h14M12 5l7 7-7 7"/>
          </svg>
        </button>
      </div>
      <div class="atp-disclaimer">Potenciado por IA — Responde en segundos</div>
    </div>
  </div>
</div>
...
:root {
  ...
  --chat-confirm: #4F9D8C;
  --chat-bg: #0C0D10;
  --chat-bg-raised: #131419;
  --chat-text: #ECE8DF;
  --chat-muted: #8B8983;
  --chat-border: rgba(236, 232, 223, 0.09);
  
  /* Premium easing */
  --chat-ease-out-strong: cubic-bezier(0.23, 1, 0.32, 1);
  --chat-spring-bounce: cubic-bezier(0.34, 1.56, 0.64, 1);
  --chat-spring-gentle: cubic-bezier(0.25, 1.46, 0.35, 0.8);
  --chat-spring-elastic: cubic-bezier(0.6, 1.2, 0.2, 1);
  --chat-active-feedback: cubic-bezier(0.16, 0.8, 0.7, 1);
}
...
.atp-chat-btn-premium {
  ...
  border-radius: 14px;
  border: none;
  background: linear-gradient(135deg, var(--chat-bg-raised) 0%, #0F1015 100%);
  ...
  transition: all 280ms var(--chat-spring-bounce), box-shadow 280ms ease;
  z-index: 10000;
}

.atp-chat-btn-premium:hover {
  transform: translateY(-4px) scale(1.05);
  box-shadow: 0 16px 48px rgba(232, 163, 61, 0.18), inset 0 1px 0 rgba(236, 232, 223, 0.12);
  border-color: rgba(232, 163, 61, 0.3);
}

.atp-chat-btn-premium:active {
  transform: scale(0.94);
  transition: transform 120ms var(--chat-active-feedback);
}
...
.atp-pulse {
  ...
  height: 12px;
  border-radius: 50%;
  background: var(--chat-confirm);
  border: 2px solid var(--chat-bg);
  animation: atp-pulse-anim 2.4s ease-in-out infinite;
}

@keyframes atp-pulse-anim {
  0%, 100% { box-shadow: 0 0 0 0 rgba(79, 157, 140, 0.7); }
  50% { box-shadow: 0 0 0 8px rgba(79, 157, 140, 0); }
}

/* ========== PANEL PREMIUM ========== */
.atp-chat-panel-premium {
  position: fixed;
  bottom: 96px;
  right: 24px;
  width: 380px;
  ...
  height: 560px;
  max-height: 75vh;
  background: linear-gradient(135deg, var(--chat-bg) 0%, #0F1015 100%);
  ...
  transition: opacity 300ms var(--chat-spring-gentle), transform 300ms var(--chat-spring-gentle);
}

.atp-chat-panel-premium.open {
  display: flex;
  opacity: 1;
...
/* ========== HEADER ========== */
.atp-chat-header-premium {
  display: flex;
  align-items: center;
  justify-content: space-between;
  padding: 18px 16px;
  border-bottom: 1px solid var(--chat-border);
  background: linear-gradient(180deg, rgba(19, 20, 25, 0.6) 0%, rgba(19, 20, 25, 0.2) 100%);
}
...
.atp-avatar-premium {
  ...
  height: 40px;
  border-radius: 12px;
  background: linear-gradient(135deg, var(--chat-accent) 0%, #D47E1F 100%);
  display: flex;
  align-items: center;
  justify-content: center;
  font-size: 20px;
  flex-shrink: 0;
  box-shadow: 0 4px 12px rgba(232, 163, 61, 0.15);
}
...
.atp-close-btn {
  ...
  padding: 6px;
  border-radius: 6px;
  transition: all 200ms var(--chat-ease-out-strong);
}
...
.atp-close-btn:active {
  transform: scale(0.92);
}

/* ========== MESSAGES ========== */
.atp-messages-container {
  flex: 1;
  overflow-y: auto;
  padding: 16px;
  display: flex;
...
.atp-messages-container::-webkit-scrollbar {
  width: 6px;
}

.atp-messages-container::-webkit-scrollbar-track {
  background: transparent;
}

.atp-messages-container::-webkit-scrollbar-thumb {
  background: rgba(236, 232, 223, 0.12);
  border-radius: 3px;
}

.atp-messages-container::-webkit-scrollbar-thumb:hover {
...
.atp-bubble {
  ...
  animation: atp-bubble-in 0.35s var(--chat-spring-bounce) forwards;
}
...
.atp-bubble:nth-child(1) { animation-delay: 0ms; }
.atp-bubble:nth-child(2) { animation-delay: 60ms; }
.atp-bubble:nth-child(3) { animation-delay: 120ms; }
.atp-bubble:nth-child(4) { animation-delay: 180ms; }

@keyframes atp-bubble-in {
  to {
    opacity: 1;
    transform: translateY(0) scale(1);
  }
...
.atp-typing span {
  ...
  border-radius: 50%;
  background: var(--chat-accent);
  animation: atp-bounce 1.4s ease-in-out infinite;
}

.atp-typing span:nth-child(2) { animation-delay: 0.2s; }
.atp-typing span:nth-child(3) { animation-delay: 0.4s; }
...
.atp-input-section {
  ...
  background: linear-gradient(180deg, rgba(19, 20, 25, 0.2) 0%, rgba(12, 13, 16, 0.4) 100%);
}
...
.atp-input-wrapper {
  ...
  transition: all 240ms var(--chat-ease-out-strong);
}

.atp-input-wrapper:focus-within {
  border-color: rgba(232, 163, 61, 0.4);
  background: rgba(19, 20, 25, 0.6);
  box-shadow: 0 0 16px rgba(232, 163, 61, 0.1), inset 0 1px 0 rgba(236, 232, 223, 0.08);
}
...
.atp-input-premium::placeholder {
  color: var(--chat-muted);
}

.atp-send-btn-premium {
  width: 32px;
  height: 32px;
  border-radius: 8px;
  border: none;
  ...
  flex-shrink: 0;
  font-weight: 600;
  transition: all 240ms var(--chat-spring-bounce), box-shadow 240ms ease;
}

.atp-send-btn-premium:hover {
  transform: translateY(-2px) scale(1.08);
  box-shadow: 0 8px 20px rgba(232, 163, 61, 0.25);
}

.atp-send-btn-premium:active {
  transform: scale(0.94);
  transition: transform 120ms var(--chat-active-feedback);
}

.atp-send-btn-premium:disabled {
  opacity: 0.5;
  cursor: not-allowed;
  transform: none;
}
...
(function() {
  ...
  const messagesContainer = document.getElementById('atp-chat-messages');
  ...
  btn.addEventListener('click', () => {
    ...
    if (!opened) {
      opened = true;
      setTimeout(() => addMessage('¡Hola! Soy Dominga, tu asistente de IA.\n\nAquí puedes hacer preguntas sobre nuestros servicios, agendar citas o solicitar más información.', 'bot'), 200);
      setTimeout(() => addMessage('¿En qué puedo ayudarte hoy?', 'bot'), 1000);
    }
    input.focus();
  });
  ...
  async function sendMessage() {
    ...
    try {
      ...
      try {
        ...
        typingEl.remove();
        addMessage('Ups, algo salió mal. Intenta de nuevo.', 'bot');
        isSending = false;
      ...
      if (!res.ok || data.error) {
        addMessage('Error técnico. Intenta más tarde.', 'bot');
        isSending = false;
        sendBtn.disabled = false;
        return;
      }
      ...
      if (botReply) {
        ...
        if (scheduleData && scheduleData.action === 'schedule') {
          addMessage('Procesando tu cita...', 'bot');
          ...
          try {
            ...
            if (scheduleRes.ok && scheduleResult.success) {
              addMessage(`✅ ${scheduleResult.message}\n\nConfirmación enviada a ${scheduleData.email}`, 'bot');
              history.push({ role: 'assistant', content: `Cita confirmada para ${scheduleData.date} a las ${scheduleData.time}` });
            } else {
              addMessage(`❌ No pude agendar. ${scheduleResult.error || 'Intenta de nuevo.'}`, 'bot');
            }
          } catch (scheduleErr) {
            addMessage('❌ Error al agendar. Intenta de nuevo.', 'bot');
          }
        } else {
          if (displayText) {
        ...
        addMessage('No pude procesar tu pregunta. Intenta de nuevo.', 'bot');
      }
    } catch (e) {
      typingEl.remove();
